...
function LoginForm() {
  ...
  // Si regresa de Google con ?token=...
  useEffect(() => {
    ...
    if (token) {
      ...
      loginWithToken(token)
        .then(() => router.push('/'))
        .catch(() => setError('Error al autenticar con Google. Intenta nuevamente.'))
  ...
  return (
    <div className="max-w-md w-full space-y-8 bg-brand-card border border-brand-border/80 p-8 sm:p-10 rounded-3xl shadow-2xl relative overflow-hidden">
      ...
      <div className="text-center space-y-2">
        <div className="inline-flex w-12 h-12 rounded-2xl bg-gradient-to-tr from-brand-primary to-brand-secondary items-center justify-center shadow-glow-primary mb-2">
        ...
        <p className="text-sm text-brand-muted">
          Ingresa a tu cuenta de <strong className="text-white">CritHit</strong> para continuar registrando tus juegos.
        </p>
      </div>
